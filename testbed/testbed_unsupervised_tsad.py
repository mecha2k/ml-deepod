...
if not args.silent_header:
    ...
    print(f"---------------------------------------------------------", file=f)
    print(
        f"data, adj_auroc, std, adj_ap, std, adj_f1, std, adj_p, std, adj_r, std, time, model",
        file=f,
    )
    f.close()
...
for dataset in dataset_name_lst:
    # # read data
    data_pkg = import_ts_data_unsupervised(
        dataset_root, dataset, entities=args.entities, combine=args.entity_combined
    )
    train_lst, test_lst, label_lst, name_lst = data_pkg

    entity_metric_lst = []
    entity_metric_std_lst = []
    for train_data, test_data, labels, dataset_name in zip(
        train_lst, test_lst, label_lst, name_lst
    ):
        entries = []
        t_lst = []
        runs = args.runs

        # using ray to tune hyperparameters
        if args.auto_hyper:
            ...
            if not hasattr(clf, "fit_auto_hyper"):
                warnings.warn(
                    f"anomaly detection model {args.model} "
            ...
            # # fit
            tuned_model_configs = clf.fit_auto_hyper(
                X=train_data,
                X_test=test_data,
                y_test=labels,
                n_ray_samples=3,
