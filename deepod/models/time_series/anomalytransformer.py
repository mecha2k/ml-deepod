...
class AnomalyTransformer(BaseDeepAD):
    ...
    def fit(self, X, y=None):
        """
        This method is used to train the AnomalyTransformer model on the provided datasets.
...
class AnomalyAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, sigma, attn_mask):
        ...
        prior = (
            self.distances.unsqueeze(0).unsqueeze(0).repeat(sigma.shape[0], sigma.shape[1], 1, 1)
        )
